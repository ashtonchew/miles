...
class PairingController:
    def __init__(
        ...
        *,
        ...
        self._engine_fleet = engine_fleet
        self._trainer_fleet = trainer_fleet
        self._layout = layout
        self._pods = pods
        self._engine_of_trainer = {
            ...
            ): lws_pod_name(fleet=engine_fleet, cell_index=cell_index, pod_index=pod_index)
    ...
    async def reconcile(self, engine_pod_name: str) -> None:
        ...
        indices = parse_lws_pod_name(pod_name=engine_pod_name, fleet=self._engine_fleet)
        if indices is None:
            return
        ...
        target = target_trainer_pod(
            ...
            layout=self._layout,
            trainer_fleet=self._trainer_fleet,
        )
    ...
    def key_of(self, pod: Any) -> str:
        ...
        if parse_lws_pod_name(pod_name=name, fleet=self._engine_fleet) is not None:
            return name
...
def parse_lws_pod_name(*, pod_name: str, fleet: str) -> PodIndices | None:
    if not pod_name.startswith(f"{fleet}-"):
        return None

    remainder = pod_name[len(fleet) + 1 :].split("-")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--engine-fleet", required=True)
    parser.add_argument("--trainer-fleet", required=True)
    ...
    asyncio.run(
        _run_forever(
            ...
            engine_fleet=args.engine_fleet,
            trainer_fleet=args.trainer_fleet,
            layout=layout,
        )
    )
    return 0
...
async def _run_forever(
    *, namespace: str, release: str, engine_fleet: str, trainer_fleet: str, layout: PairingLayout
) -> None:
    import asyncio
