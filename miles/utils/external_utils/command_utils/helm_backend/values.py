...
def colocate_section(specs: list[BaseWorkerSpec], *, layout: RunLayout) -> dict[str, Any]:
    ...
    assert len(trainers) == 1, (
        f"colocate pins engines onto one trainer fleet's nodes, but this run has {len(trainers)} trainer "
        f"fleets; which one an engine belongs beside would be undefined"
    )
    ...
    return {"enabled": True, "engineFleet": engine.name, "trainerFleet": trainer.name}
...
def _colocated_engine(engines: list[BaseWorkerSpec]) -> BaseWorkerSpec:
    ...
    assert len(declared) == 1, (
        f"colocate pins one engine fleet onto the trainer fleet's nodes, but "
        f"{[engine.name for engine in declared]} of {[engine.name for engine in engines]} say they are that "
        f"fleet; name it with --colocate-engine-spec, "
        f"because with prefill/decode disaggregation several fleets share the trainer's gpus and only the "
...
def pods_per_cell_of(spec: BaseWorkerSpec, num_gpus_per_node: int = DEFAULT_GPUS_PER_NODE) -> int:
...
def _cell_host(spec: BaseWorkerSpec, release: str, cell_index: int) -> str:
    ...
    return naming.fleet_leader_host(release, spec.name, cell_index)
