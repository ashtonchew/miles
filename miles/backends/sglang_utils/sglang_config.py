...
class SglangConfig(FrozenStrictBaseModel):
    ...
    def resolve(cls, raw: _RawSglangConfig, args) -> "SglangConfig":
        colocated_spec_name = compute_colocated_engine_spec_name(raw, args)
        ...
        model_configs = [
            ModelConfig.resolve(
                ...
                gpu_offset_cursor=gpu_offset_cursor,
                model_idx=model_idx,
                colocated_spec_name=colocated_spec_name,
            )
...
def compute_colocated_engine_spec_name(raw: _RawSglangConfig, args) -> str | None:
    if not args.colocate:
        return None

    deployed = [
        (compute_engine_spec_name(model_idx=model_idx, group_index=group_index), raw_model, raw_group)
