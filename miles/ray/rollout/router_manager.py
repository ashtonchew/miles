...
async def wait_session_server_ready(args, *, provider: BaseWorkerProvider | None):
    ...
    assert provider is not None, "--use-session-server needs the session servers' address provider"
    addrs = [
        await provider.get_addr(worker_name=session_server_worker_name(index))
