    ...
    return ServeWorkerSpec(
        ...
        ctor_kwargs=lambda ctx: dict(
            ...
            engine_provider=(
                ...
                else ctx.capability.dynamic_worker_provider(spec_names=compute_engine_spec_names(args))
            ),
            router_provider=(
                None
                if args.debug_train_only
                else ctx.capability.static_worker_provider(worker_name=compute_router_worker_name(0))
            ),
        ),
    )
