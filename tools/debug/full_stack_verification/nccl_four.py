...
def save():
    (root / f"four-{case}-{rank}.json").write_text(json.dumps(receipt, indent=2))
...
try:
    ...
    receipt["stage"] = "role_tp_allreduce_passed"
    save()
    if rank != 1:
        store = dist.PrefixStore("transfer", dist.distributed_c10d._get_default_store())
        group = (init_custom_process_group if server else init_process_group)(
            backend="nccl",
            store=store,
            rank=rank - 1 if server else 0,
            world_size=3,
