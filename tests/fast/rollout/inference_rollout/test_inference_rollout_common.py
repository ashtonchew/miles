...
async def test_group_failure_waits_for_sibling_generation_to_finish(monkeypatch) -> None:
    failure = RuntimeError("first parent failed")
    sibling_started = asyncio.Event()
    first_parent_failed = asyncio.Event()
    ...
    await sibling_started.wait()
    await first_parent_failed.wait()
