...
TRAINER_FLEET = "r-miles-run-trainer-actor"
ENGINE_FLEET = "r-miles-run-inference-engine"
...
class TestPairingLayout:
    def test_refuses_a_fleet_with_no_cells(self):
...
class TestTargetTrainerPod:
    ...
    def test_maps_a_single_pod_engine_by_dividing_and_taking_the_remainder(self):
        ...
        assert [_target(index, layout) for index in range(8)] == [
            f"{TRAINER_FLEET}-{index // 4}" if index % 4 == 0 else f"{TRAINER_FLEET}-{index // 4}-{index % 4}"
    ...
    def test_moves_on_to_the_next_trainer_cell(self):
        ...
        assert _target(4, layout) == f"{TRAINER_FLEET}-1"
    ...
    def test_names_a_cells_first_pod_without_a_rank(self):
        ...
        assert pairing.lws_pod_name(fleet=TRAINER_FLEET, cell_index=3, pod_index=0) == f"{TRAINER_FLEET}-3"
    ...
    def test_refuses_a_fleet_larger_than_the_trainer_can_seat(self):
    ...
    def test_refuses_an_engine_index_outside_the_fleet(self):
    ...
    def test_refuses_a_pod_index_outside_its_cell(self):
        """A worker index beyond the cell width means the name was parsed against the wrong fleet."""
...
class TestAssertColocateSupported:
    def test_accepts_whole_node_cells_that_tile(self):
        """What the launcher checks before rendering: whole-node pods and an engine fleet that tiles."""
...
class TestParseLwsPodName:
    def test_reads_a_leaders_indices(self):
        ...
        indices = pairing.parse_lws_pod_name(pod_name=f"{ENGINE_FLEET}-2", fleet=ENGINE_FLEET)
    ...
    def test_reads_a_workers_indices(self):
        ...
        indices = pairing.parse_lws_pod_name(pod_name=f"{ENGINE_FLEET}-2-3", fleet=ENGINE_FLEET)

        assert (indices.cell_index, indices.pod_index) == (2, 3)

    def test_returns_none_for_a_pod_of_another_fleet(self):
        """Every pod of the release comes down one stream, and most of them are not engines."""
        assert pairing.parse_lws_pod_name(pod_name=f"{TRAINER_FLEET}-0", fleet=ENGINE_FLEET) is None

    def test_returns_none_for_a_name_that_is_not_indices(self):
        """A pod merely prefixed like the fleet must not be parsed into indices it does not have."""
        assert pairing.parse_lws_pod_name(pod_name=f"{ENGINE_FLEET}-router", fleet=ENGINE_FLEET) is None
...
class TestReconcile:
    def test_releases_a_gated_engine_onto_its_trainer_node(self):
        ...
        pods = FakePods([_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False)])

        asyncio.run(_controller(pods).reconcile(f"{ENGINE_FLEET}-0"))

        assert pods.patched == [(f"{ENGINE_FLEET}-0", pairing.release_patch(node_name="gpu-3"))]
    ...
    def test_keeps_a_selector_the_pod_already_carries(self):
        ...
        engine = _pod(f"{ENGINE_FLEET}-0", node_selector={"pool": "gpu"})
        pods = FakePods([engine, _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False)])
    ...
    def test_waits_while_the_trainer_has_no_node(self):
        ...
        pods = FakePods([_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0")])
    ...
    def test_waits_while_the_trainer_does_not_exist_yet(self):
        """helm creates both fleets at once, so an engine routinely reconciles before its trainer appears."""
        pods = FakePods([_pod(f"{ENGINE_FLEET}-0")])
    ...
    def test_does_nothing_for_an_engine_already_released(self):
        ...
        pods = FakePods(
            [
                _pod(f"{ENGINE_FLEET}-0", node_name="gpu-3", gated=False),
                _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False),
            ]
        )
    ...
    def test_is_idempotent_when_run_twice(self):
        ...
        engine = _pod(f"{ENGINE_FLEET}-0")
        pods = FakePods([engine, _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False)])
        controller = _controller(pods)

        asyncio.run(controller.reconcile(f"{ENGINE_FLEET}-0"))
...
class TestKeyOf:
    def test_keys_an_engine_pod_by_itself(self):
        ...
        assert controller.key_of(_pod(f"{ENGINE_FLEET}-1")) == f"{ENGINE_FLEET}-1"
    ...
    def test_keys_a_trainer_pod_by_the_engine_it_unblocks(self):
        ...
        assert controller.key_of(_pod(f"{TRAINER_FLEET}-1")) == f"{ENGINE_FLEET}-1"

    def test_inverts_the_mapping_once_rather_than_per_event(self):
        """key_of runs for every pod on every relist, so a fleet sweep per call is quadratic."""
        controller = _controller(FakePods([]))

        assert controller.engine_waiting_on(f"{TRAINER_FLEET}-0") == f"{ENGINE_FLEET}-0"
...
class TestEventSequences:
    async def test_releases_the_engine_when_its_trainer_lands_afterwards(self):
        ...
        await harness.start(_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0"))
        assert harness.core_v1.patched == []

        await harness.upsert(_pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False))

        assert harness.core_v1.patched == [
            (f"{ENGINE_FLEET}-0", pairing.release_patch(node_name="gpu-3")),
        ]
    ...
    async def test_releases_the_engine_when_the_trainer_was_there_first(self):
        ...
        await harness.start(_pod(f"{TRAINER_FLEET}-1", node_name="gpu-9", gated=False))
        await harness.upsert(_pod(f"{ENGINE_FLEET}-1"))

        assert harness.core_v1.patched == [
            (f"{ENGINE_FLEET}-1", pairing.release_patch(node_name="gpu-9")),
        ]
        await harness.loop.stop()
    ...
    async def test_leaves_the_engine_gated_while_the_trainer_has_no_node(self):
        ...
        await harness.start(_pod(f"{ENGINE_FLEET}-0"))
        await harness.upsert(_pod(f"{TRAINER_FLEET}-0"))
    ...
    async def test_does_not_patch_twice_when_the_release_is_observed_back(self):
        ...
        await harness.start(_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False))
        await harness.upsert(_pod(f"{ENGINE_FLEET}-0", node_name="gpu-3", gated=False))

        assert harness.patched_names() == [f"{ENGINE_FLEET}-0"]
        await harness.loop.stop()
    ...
    async def test_re_enters_safely_after_a_crash_before_the_patch_landed(self):
        ...
        await first.start(_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False))
        await first.loop.stop()

        second = PairingHarness()
        await second.start(_pod(f"{ENGINE_FLEET}-0"), _pod(f"{TRAINER_FLEET}-0", node_name="gpu-3", gated=False))

        assert second.patched_names() == [f"{ENGINE_FLEET}-0"]
    ...
    async def test_resync_releases_an_engine_whose_trainer_event_was_missed(self):
        ...
        harness = PairingHarness()
        trainer = _pod(f"{TRAINER_FLEET}-0", gated=False)
        await harness.start(_pod(f"{ENGINE_FLEET}-0"), trainer)
        assert harness.core_v1.patched == []
    ...
    async def test_a_deleted_engine_is_never_patched(self):
        ...
        await harness.start(engine, _pod(f"{TRAINER_FLEET}-0"))
