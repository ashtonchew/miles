...
class TestSectionOf:
    def test_sends_a_gpu_less_spec_to_the_static_workers(self):
        """A router is never healed per cell, so a fleet would only add indirection."""
        assert values_module.section_of(_router()) == "staticWorkers"

    def test_keeps_a_single_cell_engine_a_fleet(self):
    ...
    def test_sends_a_command_fleet_to_the_engines(self):
        """An engine group is restarted as a unit and so needs a fleet."""
        assert values_module.section_of(_engine()) == "inferenceEngines"

    def test_sends_a_serve_fleet_to_the_trainers(self):
...
class TestBuildValues:
    ...
    def test_gives_a_single_cell_fleet_the_replica_count_it_asks_for(self):
        """An omitted count reads as an unmanaged field, and elastic then refuses to scale the fleet at all."""
...
class TestColocateSection:
    def test_pairs_the_fleet_a_disaggregated_run_declares(self):
        ...
        assert built["colocate"] == {
            "enabled": True,
            "engineFleet": "inference-engine-0-1",
            "trainerFleet": "trainer-actor",
        }

    def test_refuses_a_disaggregated_run_that_names_no_fleet(self):
        """Guessing the fleet from its shape found none of a prefill/decode pair, and silently so."""
    ...
    def test_refuses_two_fleets_that_both_claim_the_trainer(self):
        """One pairing controller pins one fleet, so a second claim would leave half the engines adrift."""
        specs = [
    ...
    def test_pairs_a_decode_fleet_that_leaves_trainer_gpus_to_themselves(self):
        """Prefill runs on its own nodes, so the colocated decode fleet covers only part of the trainer."""
        specs = [
        ...
        assert built["colocate"]["engineFleet"] == "inference-engine-0-0"
    ...
    def test_rejects_a_declared_fleet_whose_cell_is_smaller_than_a_node(self):
...
class TestElasticCompatibility:
    def test_a_fleet_that_started_with_one_cell_can_still_be_grown(self):
    ...
    def test_a_fleet_can_be_shrunk_back_to_one_cell(self):
...
class TestServedWorkerBootstrap:
    def test_names_the_spec_the_pod_has_to_rebuild(self):
        ...
        assert command[command.index("--spec-name") + 1] == "trainer-actor"

    def test_names_the_function_that_computes_the_constructor_keywords(self):
        """Real worker constructors are keyword-only, so handing them the argv positionally is a TypeError."""
        command = values_module.build_values([_trainer(num_cells=1, gpus_per_cell=1)], LAYOUT)["run"]["trainers"][0][
            "command"
        ]

        assert command[command.index("--ctor-kwargs-fn") + 1] == "miles.ray.specs.bootstrap.compute_ctor_kwargs"

    def test_tells_a_shared_pod_how_many_ranks_and_gpus_each_rank_has(self):
        """The ranks of one pod run the same command and tell themselves apart by this arithmetic."""
        command = values_module.build_values([_trainer(num_cells=1, gpus_per_cell=8)], LAYOUT)["run"]["trainers"][0][
            "command"
        ]

        assert command[command.index("--ranks-per-pod") + 1] == "8"
        assert command[command.index("--gpu-slots-per-rank") + 1] == "1"

    def test_keeps_the_bootstrap_flags_ahead_of_the_run_argv(self):
        """Everything after the separator belongs to the run, so a flag placed there would never be read."""
        command = values_module.build_values([_trainer(num_cells=1, gpus_per_cell=1)], LAYOUT)["run"]["trainers"][0][
            "command"
        ]

        assert command.index("--ctor-kwargs-fn") < command.index("--")
