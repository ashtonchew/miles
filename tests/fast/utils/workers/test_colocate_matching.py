...
class TestLayoutPairs:
    def test_allows_trainer_cells_that_seat_no_engine(self):
        """A prefill fleet on its own nodes leaves trainer gpus with no engine, which is a legal run."""
...
def _twelve_cell_pairing() -> Callable[[str], list[str]]:
    layout = colocate_matching.PairingLayout(
        engine_cells=12, trainer_cells=12, pods_per_engine_cell=1, pods_per_trainer_cell=1
    )
    return colocate_matching.colocated_pods_of(
        layout=layout, engine_fleet="myrun-engine", trainer_fleet="myrun-trainer", trainer_spec_name="trainer"
    )


class TestColocatedPods:
    def test_a_healed_trainer_cell_names_the_engine_pods_pinned_to_its_nodes(self):
        """A gate only holds an unscheduled pod, so a placed engine has to be deleted with its trainer."""
        layout = colocate_matching.PairingLayout(
            engine_cells=4, trainer_cells=2, pods_per_engine_cell=1, pods_per_trainer_cell=2
        )

        colocated_with = colocate_matching.colocated_pods_of(
            layout=layout, engine_fleet="myrun-engine", trainer_fleet="myrun-trainer", trainer_spec_name="trainer"
        )

        assert colocated_with("trainer-1") == ["myrun-engine-2", "myrun-engine-3"]

    def test_a_single_digit_trainer_cell_leaves_its_double_digit_namesakes_alone(self):
        """Cell 1 sharing a name prefix with cells 10 and 11 must not make healing it delete their engines."""
        colocated_with = _twelve_cell_pairing()

        assert colocated_with("trainer-1") == ["myrun-engine-1"]

    def test_a_double_digit_trainer_cell_names_only_its_own_engine(self):
        """Cells 10 and 11 each own one engine, and confusing them with cell 1 would heal the wrong pod."""
        colocated_with = _twelve_cell_pairing()

        assert colocated_with("trainer-10") == ["myrun-engine-10"]
        assert colocated_with("trainer-11") == ["myrun-engine-11"]

    def test_another_fleets_cell_drags_nothing(self):
        """Deleting engines because an unrelated cell healed would kill inference for no reason."""
        layout = colocate_matching.PairingLayout(
            engine_cells=2, trainer_cells=2, pods_per_engine_cell=1, pods_per_trainer_cell=1
        )
        ...
        assert colocated_with("router-0") == []
