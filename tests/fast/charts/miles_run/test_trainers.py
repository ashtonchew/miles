...
from tests.fast.charts.utils import objects_of_kind, render_run, requires_helm
...
def _render(trainers=ACTOR_AND_CRITIC):
    return objects_of_kind(render_run("--set-json", f"run.trainers={json.dumps(trainers)}"), "LeaderWorkerSet")
...
class TestTrainers:
    ...
    def test_gives_each_role_its_own_fleet(self):
        """An actor and a critic size their groups differently, so one fleet cannot hold both."""
    ...
    def test_names_the_container_after_its_role(self):
        """kubectl logs picks a container by name, and "worker" would not say which fleet it belongs to."""
