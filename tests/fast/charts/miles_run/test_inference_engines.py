...
from tests.fast.charts.utils import objects_of_kind, render_run, requires_helm, single_object_of_kind
...
def _render(engines=ONE_GROUP, *args):
    return render_run("--set-json", f"run.inferenceEngines={json.dumps(engines)}", *args)
...
class TestInferenceEngines:
    ...
    def test_renders_nothing_for_an_external_inference_fleet(self):
    ...
    def test_gives_every_group_of_every_model_its_own_fleet(self):
