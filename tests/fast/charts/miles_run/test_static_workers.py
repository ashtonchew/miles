...
class TestGeneratedStaticWorkerShape:
    def test_accepts_the_spec_name_the_launcher_writes_on_every_entry(self):
        """The values builder stamps specName on all three sections, so a schema without it rejects every run."""
        ...
        assert entries[0]["specName"] == "rollout-executor"
        assert objects_of_kind(_render(entries), "StatefulSet")

    def test_labels_the_pod_with_the_spec_it_implements(self):
        """A provider that observes cells by spec label has to find that label on a static worker too."""
        objects = _render([{"name": "router", "specName": "inference-router-0", "command": ["python"]}])
        labels = named_object(objects, "StatefulSet", "myrun-miles-run-router")["spec"]["template"]["metadata"][
            "labels"
        ]

        assert labels["miles.radixark.io/spec-name"] == "inference-router-0"

    def test_a_worker_without_a_spec_name_carries_no_such_label(self):
        """An entry a platform wrote by hand names no spec, and an empty label would match every selector."""
        objects = _render([{"name": "router", "command": ["python"]}])
        labels = named_object(objects, "StatefulSet", "myrun-miles-run-router")["spec"]["template"]["metadata"][
            "labels"
        ]

        assert "miles.radixark.io/spec-name" not in labels
