...
def trainer_spec(*, num_workers_per_cell: int) -> ServeWorkerSpec:
...
def cell_pods(count: int) -> list[Any]:
    return [
        make_pod(
...
def install(monkeypatch: pytest.MonkeyPatch, *, pods: list[Any], deleted: list[list[str]], ranks_per_pod: int = 1):
    ...
    return install_kubernetes_workers(
        specs=[
            trainer_spec(num_workers_per_cell=len(pods) * ranks_per_pod),
...
def installed_cells_provider(capability):
    return capability.dynamic_worker_provider(spec_names=[SPEC_NAME])
...
class TestKubernetesDriverAssembly:
    def test_the_installed_capability_answers_every_component_of_the_process(self, monkeypatch):
        ...
        capability = install(monkeypatch, pods=cell_pods(2), deleted=[])

        provider = installed_cells_provider(capability)

        assert provider is installed_cells_provider(capability)
        assert isinstance(provider, SharedK8sWorkerProvider)

    def test_refuses_to_hand_out_a_provider_for_a_fleet_it_does_not_watch(self, monkeypatch):
        ...
        with pytest.raises(AssertionError, match="not watched"):
            capability.dynamic_worker_provider(spec_names=["some-other-fleet"])
    ...
    def test_observing_a_cell_yields_rank_ordered_workers_with_handles(self, monkeypatch):
        ...
        provider = installed_cells_provider(install(monkeypatch, pods=cell_pods(3), deleted=[]))
    ...
    def test_one_pod_serving_several_ranks_yields_one_worker_per_rank(self, monkeypatch):
        ...
        provider = installed_cells_provider(install(monkeypatch, pods=cell_pods(1), deleted=[], ranks_per_pod=2))
    ...
    def test_a_trainer_cell_builds_and_drives_its_ranks_over_rpc(self, monkeypatch: pytest.MonkeyPatch):
        ...
        provider = installed_cells_provider(install(monkeypatch, pods=cell_pods(2), deleted=[]))
    ...
    def test_healing_a_cell_deletes_its_pods(self, monkeypatch):
        """Under Kubernetes a cell heals because its workload recreates the pods somebody deleted."""
        deleted: list[list[str]] = []
        capability = install(monkeypatch, pods=cell_pods(2), deleted=deleted)
    ...
    def test_the_rollout_executor_answers_over_rpc_with_no_actor_behind_it(self, monkeypatch: pytest.MonkeyPatch):
        ...
        host = static_worker_host(RELEASE, specs_rollout.ROLLOUT_EXECUTOR_SPEC_NAME)
    ...
    def test_the_rollout_components_assemble_around_those_handles(self, monkeypatch: pytest.MonkeyPatch):
        ...
        executor_host = static_worker_host(RELEASE, specs_rollout.ROLLOUT_EXECUTOR_SPEC_NAME)
        controller_host = static_worker_host(RELEASE, specs_inference.INFERENCE_CONTROLLER_SPEC_NAME)
        apps = {
    ...
    def test_the_trainer_controller_answers_over_rpc_with_no_actor_behind_it(self, monkeypatch: pytest.MonkeyPatch):
        ...
        host = static_worker_host(RELEASE, specs_train.compute_trainer_controller_spec_name("actor"))
