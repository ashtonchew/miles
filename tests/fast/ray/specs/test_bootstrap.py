...
def ctor_context(capability: FakeBackendCapability) -> WorkerCtorContext:
    return WorkerCtorContext(cell_index=0, worker_in_cell_index=0, gpu_ids=[0], capability=capability)
...
def addressing_spec() -> ServeWorkerSpec:
    return ServeWorkerSpec(
        ...
        ctor_kwargs=lambda ctx: dict(
            engine_provider=ctx.capability.cells(spec_names=WATCHED_SPEC_NAMES),
...
class TestProviderRequests:
    ...
    def test_a_spec_names_the_fleets_its_worker_will_observe(self, addressing_run):
        ...
        compute_ctor_kwargs(
            spec_name=ADDRESSING_SPEC_NAME, worker_argv=addressing_run, context=ctor_context(capability)
        )

        assert capability.requested_spec_names == [WATCHED_SPEC_NAMES]
    ...
    def test_a_spec_that_addresses_nobody_asks_the_factory_for_nothing(self, run_argv):
        ...
        compute_ctor_kwargs(spec_name=TRAINER_SPEC_NAME, worker_argv=run_argv, context=ctor_context(capability))

        assert (capability.requested_spec_names, capability.requested_worker_names) == ([], [])
