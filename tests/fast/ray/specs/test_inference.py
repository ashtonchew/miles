...
class TestSpecInferenceController:
    ...
    def test_it_renders_into_static_workers_with_its_rpc_port(self, tmp_path):
        ...
        assert entry["name"] == INFERENCE_CONTROLLER_SPEC_NAME
    ...
    def test_a_train_only_run_builds_a_controller_that_addresses_nobody(self, tmp_path):
        """--debug-train-only deploys no engines and no routers, so watching for them would never end."""
        ...
        assert (kwargs["engine_provider"], kwargs["router_provider"]) == (None, None)
