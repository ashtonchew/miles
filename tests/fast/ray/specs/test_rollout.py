...
class TestRolloutExecutorSpec:
    ...
    def test_a_train_only_run_injects_no_providers(self):
        """--debug-train-only deploys no routers, so asking the factory for one would assert."""
        context = WorkerCtorContext(
            cell_index=0, worker_in_cell_index=0, gpu_ids=[], capability=FakeBackendCapability()
        )

        kwargs = spec_rollout_executor(_args(debug_train_only=True)).ctor_kwargs(context)

        assert (kwargs["router_provider"], kwargs["session_server_provider"]) == (None, None)
