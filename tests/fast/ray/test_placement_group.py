...
def fake_components():
    ...
    with patch(
        ...
        yield Namespace(controller_handle=controller_handle, executor_handle=executor_handle)


def _providers() -> FakeBackendCapability:
    return FakeBackendCapability(static_provider=object())
...
class TestCreateRolloutComponents:
    async def test_returns_two_worker_handles(self, fake_components):
        ...
        controller, executor, _ = await create_rollout_components(args, capability=_providers())
    ...
    async def test_both_workers_are_told_to_init(self, fake_components):
        ...
        await create_rollout_components(args, capability=_providers())
    ...
    async def test_num_rollout_derived_from_executor_epoch_length(self, fake_components):
        ...
        _, _, num_rollout_per_epoch = await create_rollout_components(args, capability=_providers())
    ...
    async def test_a_train_only_run_resolves_no_inference_addresses(self, fake_components):
        ...
        capability = _providers()

        await create_rollout_components(args, capability=capability)

        assert capability.requested_worker_names == []
