...
| `--colocate-engine-spec` | derived | Engine spec sharing the trainer's GPUs, e.g. `inference-engine-0-1`; needed only when several fleets could be it. |
...
| `--colocate-engine-spec` | str | derived | Name of the engine spec that shares the trainer's GPUs, e.g. `inference-engine-0-1`. Defaults to the decode fleet of the weight-updated model, and is only needed when several fleets could be the colocated one. |
