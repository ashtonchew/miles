...
run:
  ...
  inferenceEngines: [{name: engine, replicas: 4, size: 2, command: [...], resources: {limits: {nvidia.com/gpu: 8}}, specName: engine, meta: {...}}]
...
- Pods of one cell must be identifiable as such and their ranks ordered.
    - Miles reads the label keys defined in `miles/utils/workers/worker_provider/k8s_labels.py`,
...
- The command names the spec (`--spec-name`) and a function that turns the run's argv back into
...
- The cell index comes from the group index in `LWS_LEADER_ADDRESS`; a pod outside a fleet is
...
- The orchestrator computes them from the specs it already holds, per observed cell index, so a
  fact that differs between the cells of one fleet stays correct even though the fleet renders
...
`run.colocate.engineFleet` is the fleet the run declares colocated, never one guessed from
fleet sizes: with prefill/decode disaggregation several engine fleets share the trainer's
gpus, and only the declared one is paired cell for cell. The launcher takes it from the
engine spec `--colocate-engine-spec` names, which defaults to the decode fleet of the
weight-updated model.

Trainer gpus that seat no engine are legal: with prefill/decode disaggregation the prefill
fleet runs on its own nodes, so the colocated fleet covers only part of the trainer. These
...
- More engine cells than the trainer fleet can seat.
...
- Engine pods are born with a `miles.radixark.io/colocate-pairing` scheduling gate and no
  affinity.
- A single-replica controller watches both fleets and maps each gated engine pod to the
